...
# 定义全局变量
ip_groups="192.168.200.2 192.168.200.3"
...
backend kubernetes-apiserver
    mode        tcp
    balance     roundrobin
    server master1 192.168.200.1:6443 check
    server master2 192.168.200.2:6443 check
    server master3 192.168.200.3:6443 check
EOF
...
do
    ...
    ssh root@$file sed -i 's/100/90/g' /etc/keepalived/keepalived.conf
    ssh root@$file sed -i 's/100/80/g' /etc/keepalived/keepalived.conf
done
